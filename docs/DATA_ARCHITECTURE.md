...
- **`CURATED`** (`fleet/curated.py`) — the hand-researched metadata table:
  optimiser, region and MWh for the sites it covers. The registry generator
  copies this metadata into `REGISTRY`.
- **`REGISTRY`** (`fleet/registry.py`) — the fleet this project analyses: every
  BM-registered battery the census identifies whose energy capacity is
  published, generated by `scripts/build_registry.py`. Generated, not written,
  so it is a measurement. This is what the live dashboard renders.
- **`census_population()`** — every BM-registered battery the census can
  identify, with machine-derived rather than hand-checked metadata.

Each module states its own size; none is quoted here, because the census is
rebuilt from live registers and any count written down in this document would go
stale the next time it runs.

**`CURATED` is the default argument of every fetcher and every metric** — see
the `population: Population = CURATED` signatures in `fleet/fetch_fleet.py` and
`fleet/performance.py`. That default exists so a caller which says nothing keeps
writing to the cache directory it always wrote to, not because it is the
preferred population. The dashboard passes `REGISTRY` explicitly
(`from fleet.registry import REGISTRY as FLEET_POPULATION`) and the notebooks
pass the census explicitly. No metric is duplicated, reimplemented or removed,
and a metric added for one tier is available to both.
