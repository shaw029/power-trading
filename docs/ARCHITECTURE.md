...
**The Objective:** The strategy takes directional exposure in the **GB day-ahead (DA) auction price series** based on expected system imbalance, then manages exit through a two-slice hybrid engine:
...
The virtual research strategy predicts the future cash-out-minus-DA spread before the auction. Its canonical signal study holds positions to imbalance settlement; notebook 02 separately explores stylised MID exits. Cash-out is a settlement mechanism, not a quoted execution venue. Current cleared DA and MID prices are outcomes, not pre-auction signal inputs.
...
* **Leakage Prevention (enforced, not asserted):** The DA auction for market date D clears at 11:00 on D-1, and the book must be committed before it; the feature vintage used throughout is D-1 10:30. Forecast features are taken at that cutoff. Lagged series use a **96-period (48-hour)** offset, which is the shortest same-time-of-day lag that stays pre-auction for *every* delivery period of the day — the last period of D looks back to D-2 23:30, still 11 h before the auction.

    A 24-hour offset does **not** clear that bar and was previously used. For a delivery period at D 20:00 it reads the D-1 20:00 print, nine and a half hours *after* the decision; across the 2018 sample that is true of 9,542 of 17,616 rows — every period delivered after 10:30. `src/features/build_features.py` now derives each row's own auction decision time and raises if any lagged column reads past it (`assert_auction_available`), so the property is checked on the frame actually written rather than promised in prose.
...
* **Long vs Short — Direction Rule:** The model predicts `spread = SSP − DA_price` for each half-hour settlement period. A **LONG** (buy DA, settle at SSP) fires when `predicted_spread > gate`; a **SHORT** (sell DA, settle at SBP) fires when `predicted_spread < −gate`. The gate is `max(execution_buffer, 0) + max(threshold, vol_multiplier × vol_threshold)`, where `execution_buffer` is the known round-trip cost (fees plus spread crossing, and only when the position can actually be closed intraday), `threshold` is the configured minimum floor, and `vol_threshold` is a lagged rolling standard deviation of the **cash-out-to-auction gap**. The buffer used to be a rolling mean of SBP − SSP standing in for imbalance penalty, and the volatility term a rolling std of the same difference; under GB's single cash-out price both are identically zero in every settlement period, so the buffer contributed nothing and the "adaptive" gate never moved off its floor. A long is the view that the grid will be short — SSP will exceed the DA clearing price. A short is the view that the grid will be long — SBP will be below DA, allowing a buy-back at a cheaper imbalance price.
...
All features are constructed from the D-1 10:30 pre-auction forecast vintage. No same-day actuals are used; lagged inputs apply a 96-period (48-hour) minimum offset, asserted per row against that row's own auction decision time.
...
| **Historical Lags** | `day_ahead_price_lag96`, `day_ahead_price_lag144`, `system_buy_price_lag96`, `system_buy_price_lag144`, `imbalance_gap_lag96`, `imbalance_gap_lag144` | 48 h and 72 h lookbacks on the DA price, the imbalance cash-out price, and the gap between them. **Only one cash-out leg appears.** GB has settled on a *single* imbalance price since BSC P305 (5 November 2015): SBP and SSP are the same number in every settlement period — all 17,660 non-null pairs in the 2018 sample are identical — so carrying both legs double-counts one signal, and the old `SBP − SSP` "spread" feature was identically zero. The quantity that actually risks a day-ahead position is cash-out minus the cleared auction price, so that is the lagged risk feature. 96 periods is the minimum offset that stays pre-auction for every delivery period. |
