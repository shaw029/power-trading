...
FINDING = {
    ...
    **£6/MW/day, about 6% of benchmark revenue**. A profit-optimal 50 MW / 2 h asset
    ...
    "05": ("Stress Response Study — full history, DRM < 1 GW", "scarcity", """
    ...
    **48% dispatch gap**. Responding is not the problem; being ready is."""),
    ...
    "07": ("Regime Shift — did the fleet change, or the control room?", "scarcity", """
    ...
    "08": ("Stress Response, Modern Era — from April 2024", "scarcity", """
    ...
    "09": ("The model and the fleet, on one yardstick", "utilisation", """
    ...
    **Window limit, stated up front:** all of this is 60 days, late June to late
    ...
    "10": ("What the Balancing Mechanism does to every fleet figure", "both", """
...
def extract_charts() -> list[dict]:
    ...
    for path in sorted(NB_DIR.glob("[01][0-9]*.ipynb")):
        ...
        for ci, cell in enumerate(nb["cells"]):
            ...
            titles = (re.findall(r'set_title\(\s*[rf]?["\']([^"\']{4,120})', src)
                      + re.findall(r'suptitle\(\s*[rf]?["\']([^"\']{4,120})', src))
            ...
            for j, out in enumerate(imgs):
                ...
                manifest.append({
                    "nb": nbid, "file": f"{name}.png", "cell": ci, "section": heading,
...
def main() -> None:
    ...
    metrics = {p.name[2:4]: json.loads(p.read_text())
               for p in sorted(POSTER.glob("nb*_metrics.json"))}
    L: list[str] = []
    w = L.append
    ...
    w(f"| Contents | {sum(len(m) for m in metrics.values())} exported metrics · "
      f"{sum(len(c.splitlines()) for c in CONSOLE.values()):,} lines of console output · "
      f"{len(charts)} charts |\n")
    w("Notebooks 01–03 export no poster metrics and are excluded: they are the "
      "superseded strategy-development lineage.\n")

    w("## The two lanes — read before comparing any two numbers\n")
    w("The commonest error here is quoting a utilisation number as a resilience "
      "finding. The lanes use different rulers and are not commensurable.\n")
    w("| Lane | Question | Threshold | Notebooks |\n|---|---|---|---|")
    w("| **Utilisation / incentives** | What happens when the system works *hardest*? "
      "| top-decile residual load (23.3 GW) | **04, 09** |")
    w("| **Operator scarcity** | What happens when the operator has *least slack*? "
      "| `DRM < 1 GW`, CMN, LoLP | **05, 07, 08** |")
    w("| **Population** | How much fleet is there, how well measured? | — | **06** |\n")
    w("Top-decile load is **not** a shortage. No number from 04 or 09 may be quoted "
    ...
    w(f"| **total** | **{len(charts)}** | **{sum(1 for c in charts if c['poster'])}** "
    ...
    for nb in sorted(FINDING):
        ...
        w(f"### Console output — every number this notebook printed "
        ...
        w(f"### Charts ({len(rows)})\n")
        if not rows:
            w("> **None.** This notebook produces no figures — a real gap for a "
    ...
    w("| Top-decile threshold | 23.3 GW, 2,878 half-hours | printed by **both** 04 and 09 — same function |")
    w("| Cycling rule | 0.3 cycles/day | `ANCILLARY_CYCLES_THRESHOLD`, one definition, test-guarded |")
    w("| Poster figures | read from `nb*_metrics.json` | `poster.typ` substitutes; never retyped |\n")
    w("## Known gaps\n")
    w("1. **nb08 has no charts.** It carries a headline finding (modern-era response, "
      "LOLE 1.0 h) with nothing to show for it.\n")
    w("2. **nb09 has no winter cut.** Its 60 days are late June–August; whether the "
      "80/40 ancillary gradient holds against a January peak is untested.\n")
    w("3. **The nb09 mechanism is unproven.** The ancillary association is robust and "
    ...
    w("4. **The poster does not yet carry nb09.** Its metrics file is readable by "
      "`poster.typ`, but no panel references it.\n")
    w("5. **`nb04_fig6_money_vs_coverage` is an orphan** — byte-identical to "
      "`nb04_fig_money_vs_coverage`, left behind by a rename, referenced nowhere.\n")
    w("6. **`stress` survives as a code identifier**, deliberately: printed labels in "
    ...
    w("> **This section is editorial.** Every claim above it is measured and traceable "
    ...
    w("")
    w("**The honest caveat on all seven.** Items 1–3 and 7 rest on a single 60-day "
    ...
    w("The riskiest numbers here are not the wrong ones, they are the ones whose "
    ...
    for weak, breaks, holds in [
        ...
        ("Anything from 04 or 09",
         ...
         "**when the system is working hardest** — utilisation lane, separate from "
         "05/07/08"),
        ("nb07 regime shift",
    ...
    for i, (fix, why, cost) in enumerate([
        ("**Give nb08 charts.** It carries the modern-era headline — response +0.121, "
         ...
         "cannot use a finding it cannot draw.", "~1 h"),
        ("**Add nb09's winter cut.** Recompute the 46/85/80/40 split on a winter window "
         ...
         "sensitivity; nb09 only inherits the exposure.", "~2 h, no refetch"),
        ("**Pull per-unit contracted MW by service from NESO auction results.** Then "
         ...
         "from association to mechanism.", "~half a day"),
        ("**Point the poster at nb09.** `nb09_metrics.json` exists and `poster.typ` can "
         ...
         "argument the poster currently lacks.", "~1 h"),
        ...
        ("**Leave `stress` alone as a code identifier.** Printed labels are already "
         ...
         "files.", "Listed so the decision is recorded rather than rediscovered. Tier 1 "
         "(the column key, ~30 min) is optional; tiers 2–3 cost a store rebuild.", "0"),
    ], start=1):
        w(f"| {i} | {fix} | {why} | {cost} |")
    w("")
    ...
    OUT.write_text("\n".join(L) + "\n")
    print(f"wrote {OUT.relative_to(HERE.parent)} — {sum(len(m) for m in metrics.values())} "
