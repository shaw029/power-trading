...
| `08_stress_response_modern.ipynb` | **Notebook 05's measurements on the modern fleet only**, from 2024-04-01, because notebook 07 shows the eight-year window blends two regimes and is weighted toward the old one. Same store, same conditioning sets, thresholds recomputed within the era. The binding constraint reverses: the dispatch gap falls 52% → 32% while the preparedness gap climbs 35% → 52% and state of charge at onset drops 62% → 47%. The fleet responds twice as hard and starts emptier |
