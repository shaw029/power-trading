...
65-site population — every BM-registered battery with a known energy capacity, 88% of
...
states its own coverage on the page (88% of GB BM-registered battery MW) and points at
...
under. Three exist — `REGISTRY` (the curated 23 sites / 47 BM Units), `EXTENDED`
(the dashboard's 65 sites / 99 BM Units, generated by
`scripts/build_extended_fleet.py`) and
...
| `fleet/registry.py` | 23 curated sites, hand-verified — the metadata baseline | presentation — **unchanged** |
| `fleet/extended.py` | 65 sites with known MWh, generated from the census — **the dashboard's population** | presentation |
...
*The table above is the **curated registry's** coverage, and it is why the dashboard no
longer runs on the registry alone. Since August 2026 it runs `fleet/extended.py` — the
65 census sites with a known energy capacity — which lifts coverage to **88% of GB
BM-registered battery MW**: 100% above 100 MW, 97% of 50–100 MW, 50% of 20–50 MW and 11%
below 20 MW. The registry remains the hand-verified metadata baseline and a subset of it.*
