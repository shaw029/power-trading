// Beyond the Spread, lean variant. A0 landscape (1189 x 841 mm).
//
// Same analysis, same metric files, same numbers as poster.typ. What differs
// is reading design. The dense board carries ~2,290 words, which is a nine
// minute read; a poster session gives five seconds walking past, sixty
// seconds if the reader stops, and after that the author is the medium.
//
// So this variant builds the middle tier the dense board lacks. Every finding
// opens with a bold statement line that stands on its own, and the supporting
// prose beneath it is two or three lines. A reader who takes only the bold
// lines leaves with the whole argument; a judge who reads everything still
// finds the caveats. Material that exists to defend rather than to inform
// moves behind the QR.
//
