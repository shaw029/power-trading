...
Run from the repo root. Expect several minutes on a warm cache.
"""
...
def _publish(**kv):
    ...
    The poster reads this file the same way it reads the notebooks' exports, so
    a BOALF figure on the board is never typed. Scripts rather than notebooks
    produce it only because the notebooks do not yet net acceptances; when they
    do, these keys should move into their export cells and this file can go.
    """
    import json
    from pathlib import Path as _P
    p = _P("reports/figures/poster/boalf_metrics.json")
...
r.to_csv("reports/boalf_readiness.csv", index=False)
print("\nsaved -> reports/boalf_readiness.csv")
